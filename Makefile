...
mpm-cola1.x: main-cola1.cpp $(INCLUDE_FILE_PATHS)
	$(CXX) -o $@ $< -IGraMPM/include -Iinclude -I$(INCLUDEDIR) $(CXXFLAGS) -I$(KOKKOS_ROOT)/include -L$(KOKKOS_ROOT)/lib -lkokkoscore

tests: tests/interfaces.x tests/kernels.x tests/grid_mapper.x tests/boundary_conditions.x tests/p2g.x tests/g2p.x tests/stress_update.x tests/updates.x tests/mpi.x
