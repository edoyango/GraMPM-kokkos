        ...
        namespace functors {
            namespace stress_update {
                ...
                struct drucker_prager_elastoplastic {
                    double dt, phi, psi, cohesion, alpha_phi, alpha_psi, k_c, E, v, D0;
                    const Kokkos::View<F*[6]> p_sigma, p_strainrate;
                    const Kokkos::View<F*[3]> p_spinrate;
                    ...
                    void set_DP_params(F phi_, F psi_, F c_, F E_, F v_) {
                        ...
                        psi = psi_;
                        cohesion = c_;
                        alpha_phi = 2.*std::sin(phi)/(std::sqrt(3.)*(3.-std::sin(phi)));
                        alpha_psi = 2.*std::sin(psi)/(std::sqrt(3.)*(3.-std::sin(phi)));
                        k_c = 6.*cohesion*std::cos(phi)/(std::sqrt(3.)*(3.-std::sin(phi)));
                        E = E_;
                        v = v_;
                        D0 = E/((1.+v)*(1.-2.*v));
                    }
                    ...
                    void operator()(const int i) const {
                        ...
                        if (J2 == 0. && I1 > k_c/alpha_phi) {
                            p_sigma(i, 0) = k_c/alpha_phi/3.;
                            p_sigma(i, 1) = k_c/alpha_phi/3.;
                            p_sigma(i, 2) = k_c/alpha_phi/3.;
                            I1 = k_c/alpha_phi;
                        }
                        ...
                        if (f > 1.e-13) {
                            const F snorm = 2.*Kokkos::sqrt(J2);
                            const F shat[6] {
                                s[0]/snorm,
                                s[1]/snorm,
                                s[2]/snorm,
                            ...
                            p_sigma(i, 0) -= dlambda*D0*((1.-v)*dgdsig[0] + v*dgdsig[1] + v*dgdsig[2]);
                            p_sigma(i, 1) -= dlambda*D0*(v*dgdsig[0] + (1.-v)*dgdsig[1] + v*dgdsig[2]);
                            p_sigma(i, 2) -= dlambda*D0*(v*dgdsig[0] + v*dgdsig[1] + (1.-v)*dgdsig[2]);
                            p_sigma(i, 3) -= dlambda*D0*dgdsig[3]*(1.-2.*v);
                            p_sigma(i, 4) -= dlambda*D0*dgdsig[4]*(1.-2.*v);
                            p_sigma(i, 5) -= dlambda*D0*dgdsig[5]*(1.-2.*v);
