        ...
        namespace functors {
            ...
            struct map_gidx {
                ...
                void operator()(const int i) const {
                    gidx(i, 0) = static_cast<int>((x(i, 0)-ming[0])/dcell);
                    gidx(i, 1) = static_cast<int>((x(i, 1)-ming[1])/dcell);
                    gidx(i, 2) = static_cast<int>((x(i, 2)-ming[2])/dcell);
                }
            };
            ...
            struct find_neighbour_nodes {
                ...
                find_neighbour_nodes(F dcell_, const F ming_[3], const int ng_[3], int r, Kokkos::View<F*[3]> x_, 
                    Kokkos::View<int*[3]> gidx_, Kokkos::View<int*[3]> pg_, Kokkos::View<F*> w_, Kokkos::View<F*[3]> dwdx_, 
                    kernel knl_)
                    : dcell {dcell_}
                    , ming {ming_[0], ming_[1], ming_[2]}
                    , ng {ng_[0], ng_[1], ng_[2]}
                ...
                KOKKOS_INLINE_FUNCTION
                void operator()(const int i) const {
                    const int idxx = gidx(i, 0), idxy = gidx(i, 1), idxz = gidx(i, 2);
                    int j = i*npp;
                    for (int jdxx = idxx+1-knl.radius; jdxx <= idxx+knl.radius; ++jdxx) {
                        for (int jdxy = idxy+1-knl.radius; jdxy <= idxy+knl.radius; ++jdxy) {
                            for (int jdxz = idxz+1-knl.radius; jdxz <= idxz+knl.radius; ++jdxz) {
                                ...
                                pg(j, 1) = jdxy;
                                pg(j, 2) = jdxz;
                                const F dx = x(i, 0) - (jdxx*dcell+ming[0]), 
                                    dy = x(i, 1) - (jdxy*dcell+ming[1]), 
            ...
            struct map_p2g_mass {
                ...
                const Kokkos::View<int*[3]> pg;
                const Kokkos::View<F*> w;
                map_p2g_mass(int npp_, Kokkos::View<F*> p_mass_, Kokkos::View<F***> g_mass_, Kokkos::View<int*[3]> pg_, 
                    Kokkos::View<F*> w_)
                    : npp {npp_}
                    , p_mass {p_mass_}
                    , g_mass {g_mass_}
                ...
                void operator()(const int i) const {
                    ...
                    for (int j = jstart; j < jstart + npp; ++j) {
                        ...
                        g_mass(pg(j, 0), pg(j, 1), pg(j, 2)) += massi*w(j);
                    }
                }
            };
            ...
            struct map_p2g_momentum {
                ...
                map_p2g_momentum(int npp_, Kokkos::View<F*> p_mass_, Kokkos::View<F*[3]> p_v_, 
                    Kokkos::View<F***[3]> g_momentum_, Kokkos::View<int*[3]> pg_, Kokkos::View<F*> w_)
                    : npp {npp_}
                    , p_mass {p_mass_}
                    , p_v {p_v_}
                ...
                void operator()(const int i) const {
                    ...
                    for (int j = jstart; j < jstart + npp; ++j) {
                        const int idxx = pg(j, 0), idxy = pg(j, 1), idxz = pg(j, 2);
                        const F massiwj = massi*w(j);
                        // Kokkos::atomic_add(&g_mass(idx), p_mass(i)*w(j));
                        g_momentum(idxx, idxy, idxz, 0) += vxi*massiwj;
                        g_momentum(idxx, idxy, idxz, 1) += vyi*massiwj;
                        g_momentum(idxx, idxy, idxz, 2) += vzi*massiwj;
                    }
                }
            };
            ...
            struct map_p2g_force {
                ...
                map_p2g_force(int npp_, Kokkos::View<F*> p_mass_, Kokkos::View<F*> p_rho_, 
                    Kokkos::View<F*[6]> p_sigma_, Kokkos::View<F***[3]> g_force_, Kokkos::View<int*[3]> pg_, 
                ...
                void operator()(const int i) const {
                    ...
                    for (int j = jstart; j < jstart + npp; ++j) {
                        ...
                        g_force(idxx, idxy, idxz, 0) += -vi*(
                            sigixx*dwdxj +
                            sigixy*dwdyj +
                            sigixz*dwdzj
                        ) + bfx*massiwj;
                        g_force(idxx, idxy, idxz, 1) += -vi*(
                            sigixy*dwdxj +
                            sigiyy*dwdyj +
                            sigiyz*dwdzj
                        ) + bfy*massiwj;
                        g_force(idxx, idxy, idxz, 2) += -vi*(
                            sigixz*dwdxj +
                            sigiyz*dwdyj +
                            sigizz*dwdzj
                        ) + bfz*massiwj;
            ...
            struct map_g2p_acceleration {
                ...
                const Kokkos::View<F***[3]> g_force, g_momentum;
                ...
                map_g2p_acceleration(int npp_, Kokkos::View<F*[3]> p_a_, Kokkos::View<F***[3]> g_force_, 
                    ...
                    Kokkos::View<F*> w_, Kokkos::View<int*[3]> pg_)
                    : npp {npp_}
                    , p_a {p_a_}
                    , g_force {g_force_}
                    , p_dxdt {p_dxdt_}
                ...
                KOKKOS_INLINE_FUNCTION
                void operator()(const int i) const {
                    F axi = F(0.), ayi = F(0.), azi = F(0.);
                    F dxdti = F(0.), dydti = F(0.), dzdti = F(0.);
                    const int jstart = i*npp;
                    for (int j = jstart; j < jstart + npp; ++j) {
                        const int idxx = pg(j, 0), idxy = pg(j, 1), idxz = pg(j, 2);
                        const F wjmassj = w(j)/g_mass(idxx, idxy, idxz);
                        axi += g_force(idxx, idxy, idxz, 0)*wjmassj;
                        ayi += g_force(idxx, idxy, idxz, 1)*wjmassj;
                        azi += g_force(idxx, idxy, idxz, 2)*wjmassj;
                        dxdti += g_momentum(idxx, idxy, idxz, 0)*wjmassj;
                        dydti += g_momentum(idxx, idxy, idxz, 1)*wjmassj;
                        dzdti += g_momentum(idxx, idxy, idxz, 2)*wjmassj;
                    }
                    p_a(i, 0) = axi;
                    p_a(i, 1) = ayi;
                    p_a(i, 2) = azi;
            ...
            struct map_g2p_strainrate {
                ...
                const Kokkos::View<F*[3]> p_spinrate, dwdx;
                const Kokkos::View<F***[3]> g_momentum;
                const Kokkos::View<F***> g_mass;
                const Kokkos::View<int*[3]> pg;

                map_g2p_strainrate(int npp_, Kokkos::View<F*[6]> p_strainrate_, 
                    Kokkos::View<F*[3]> p_spinrate_, Kokkos::View<F***[3]> g_momentum_,
                    Kokkos::View<F*[3]> dwdx_, Kokkos::View<F***> g_mass_, Kokkos::View<int*[3]> pg_)
                    : npp {npp_}
                ...
                KOKKOS_INLINE_FUNCTION
                void operator()(const int i) const {
                    F strainratexxi = F(0.), strainrateyyi = F(0.), strainratezzi = F(0.), strainratexyi = F(0.), 
                        strainratexzi = F(0.), strainrateyzi = F(0.), spinratexyi = F(0.), spinratexzi = F(0.), 
                        spinrateyzi = F(0.);
                    const int jstart = i*npp;
                    for (int j = jstart; j < jstart + npp; ++j) {
                        const int idxx = pg(j, 0), idxy = pg(j, 1), idxz = pg(j, 2);
                        const F massj = g_mass(idxx, idxy, idxz), dwdxjmassj = dwdx(j, 0)/massj, 
                            dwdyjmassj = dwdx(j, 1)/massj, dwdzjmassj = dwdx(j, 2)/massj, 
                            momentumxj = g_momentum(idxx, idxy, idxz, 0), momentumyj = g_momentum(idxx, idxy, idxz, 1), 
                            momentumzj = g_momentum(idxx, idxy, idxz, 2);
            ...
            struct update_particle_data {
                ...
                update_particle_data(Kokkos::View<F*[3]> data_, Kokkos::View<F*[3]> update_)
                    : data {data_}
                    , update {update_}
                {}
            ...
            template<typename F>
            struct update_grid_data {
                double dt;
                const Kokkos::View<F***[3]> data, update;

                update_grid_data(Kokkos::View<F***[3]> data_, Kokkos::View<F***[3]> update_)
                    : data {data_}
                    , update {update_}
                {}
                ...
                void operator()(const int i, const int j, const int k) const {
                    data(i, j, k, 0) += dt*update(i, j, k, 0);
                    data(i, j, k, 1) += dt*update(i, j, k, 1);
                    data(i, j, k, 2) += dt*update(i, j, k, 2);
                }
            };
