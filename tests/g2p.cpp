...
TEST(g2p_strainspinrates, cubic_bspline) {
    const double dcell = 0.2;
    std::array<double, 3> mingrid {-dcell, -dcell, -dcell}, maxgrid {0.99+dcell, 1.99+dcell, 2.99+dcell};
    ...
    EXPECT_DOUBLE_EQ(myMPM.p_spinratexy(0), 0.05);
    EXPECT_DOUBLE_EQ(myMPM.p_spinratexz(0), 0.1);
    EXPECT_NEAR(myMPM.p_spinrateyz(0), 0.05, 5.e-17);

    EXPECT_NEAR(myMPM.p_strainratexx(myMPM.p_size()-1), 0.1, 2.e-15);
    EXPECT_NEAR(myMPM.p_strainrateyy(myMPM.p_size()-1), 0.2, 2.e-15);
    EXPECT_DOUBLE_EQ(myMPM.p_strainratezz(myMPM.p_size()-1), 0.3);
    EXPECT_NEAR(myMPM.p_strainratexy(myMPM.p_size()-1), -0.15, 2.e-15);
    EXPECT_NEAR(myMPM.p_strainratexz(myMPM.p_size()-1), -0.2, 2.e-15);
    EXPECT_NEAR(myMPM.p_strainrateyz(myMPM.p_size()-1), -0.25, 2.e-15);
    EXPECT_NEAR(myMPM.p_spinratexy(myMPM.p_size()-1), 0.05, 2.e-15);
    EXPECT_NEAR(myMPM.p_spinratexz(myMPM.p_size()-1), 0.1, 2.e-15);
    EXPECT_NEAR(myMPM.p_spinrateyz(myMPM.p_size()-1), 0.05, 2.e-15);
}
